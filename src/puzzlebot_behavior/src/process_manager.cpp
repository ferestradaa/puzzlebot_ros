    ...
    public:
        ...
        ~ProcessManager() {
            ...
            for (auto& [name, pid] : processes_) {
                RCLCPP_INFO(get_logger(), "cleanup: killing %s (pid %d)", name.c_str(), pid);
            ...
            std::shared_ptr<rclcpp_action::ServerGoalHandle<LaunchNode>> handle)
        {
            std::thread([this, handle]() {
                ...
                while (rclcpp::ok() && !node_ready) {
                    ...
                    fb->status = "waiting readiness";
                    handle->publish_feedback(fb);
                    std::this_thread::sleep_for(std::chrono::milliseconds(200));
                }

                RCLCPP_INFO(get_logger(), "%s ready", goal->executable.c_str());
                result->success = true;
                result->pid = pid;
    ...
    int main(int argc, char** argv)
    {
        ...
        {
            auto node = std::make_shared<ProcessManager>();
            rclcpp::spin(node);
        }  
        rclcpp::shutdown();
        return 0;
