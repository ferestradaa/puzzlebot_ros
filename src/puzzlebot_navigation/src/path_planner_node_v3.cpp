...
public:
    ...
    rng_(std::random_device{}())
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "Path planner ready | map_topic=%s | service=/plan_path | tf=%s->%s | robot_radius=%.2f",
...
private:
  ...
  {
    return line_is_free_internal(a, b, false, false);
  }

  bool line_is_free_ignore_endpoints(const GridCell & a, const GridCell & b) const
  {
    return line_is_free_internal(a, b, true, true);
  }
    ...
    bool ignore_goal) const
  {
    ...
    while (true) {
      ...
      const bool should_skip_cell_check =
        (ignore_start && at_start) || (ignore_goal && at_goal);

      if (!should_skip_cell_check && !is_cell_free({x0, y0})) {
        return false;
      }
