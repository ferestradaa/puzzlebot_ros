...
private:
    ...
    const geometry_msgs::msg::PoseStamped & goal_msg)
  {
    ...
    const GridCell start_grid_raw = world_to_grid(robot_x, robot_y);
    const GridCell goal_grid_raw  = world_to_grid(goal_x, goal_y);

    const GridCell start_grid = snap_to_free(start_grid_raw, start_free_search_radius_cells_);
    const GridCell goal_grid  = snap_to_free(goal_grid_raw,  start_free_search_radius_cells_);

    if (!is_cell_free(start_grid)) {
      return {false, "Start position is in obstacle even after free-cell search.", nav_msgs::msg::Path()};
    }
    if (!is_cell_free(goal_grid)) {
      return {false, "Goal position is in obstacle even after free-cell search.", nav_msgs::msg::Path()};
    }

    if (start_grid != start_grid_raw) {
      RCLCPP_WARN(
        get_logger(),
        "Start cell (%d,%d) was in obstacle, snapped to (%d,%d).",
        start_grid_raw.first, start_grid_raw.second,
        start_grid.first, start_grid.second);
    }
    if (goal_grid != goal_grid_raw) {
      RCLCPP_WARN(
        get_logger(),
        "Goal cell (%d,%d) was in obstacle, snapped to (%d,%d).",
        goal_grid_raw.first, goal_grid_raw.second,
