...
public:
  ...
  : Node("dynamic_map_node")
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "dynamic_map_node ready | waiting for static map on %s | pub_hz=%.1f | tf: %s->%s",
      static_map_topic_.c_str(),
      1000.0 / static_cast<double>(map_publish_period_ms_),
      global_frame_.c_str(),
      robot_frame_.c_str());
  }
...
private:
  void declare_and_load_parameters()
  {
    ...
    declare_parameter<double>("scan_angle_offset",  0);
    ...
    scan_step_ =
      std::max(1, static_cast<int>(get_parameter("scan_step").as_int()));
    ...
    dynamic_hit_increment_   = static_cast<float>(get_parameter("dynamic_hit_increment").as_double());
    dynamic_miss_decay_      = static_cast<float>(get_parameter("dynamic_miss_decay").as_double());
    dynamic_lock_threshold_  = static_cast<float>(get_parameter("dynamic_lock_threshold").as_double());
    dynamic_max_score_       = static_cast<float>(get_parameter("dynamic_max_score").as_double());
  ...
  void static_map_callback(const nav_msgs::msg::OccupancyGrid::SharedPtr msg)
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "Static map received | size=(%d,%d) | res=%.4f | origin=(%.3f, %.3f)",
      width_, height_, resolution_, origin_[0], origin_[1]);
  }
  ...
  // Obtiene pose del robot via TF lookup (global_frame_ -> robot_frame_)
  // Retorna false si el lookup falla; no bloquea el spin
  ...
  void scan_callback(const sensor_msgs::msg::LaserScan::SharedPtr msg)
  {
    if (!has_static_map_) {
      RCLCPP_WARN_THROTTLE(
        ...
        "No static map received yet on %s.", static_map_topic_.c_str());
      return;
    }
    ...
    if (!scan_processor_.valid()) {
      RCLCPP_WARN_THROTTLE(
        ...
        "Invalid scan: only %d valid points.", scan_processor_.valid_points());
      return;
    }
  ...
  void integrate_dynamic_obstacles_only(const puzzlebot_navigation::Pose2D & pose)
  {
    ...
    for (std::size_t i = 0; i < scan_x.size(); ++i) {
      ...
      dynamic_score_[ks] = std::min(
        dynamic_max_score_,
        dynamic_score_[ks] + dynamic_hit_increment_);
    }
  }
  ...
  void decay_unseen_obstacles()
  {
    for (std::size_t i = 0; i < dynamic_score_.size(); ++i) {
      ...
      if (dynamic_score_[i] <= 0.0f) {
        dynamic_score_[i] = 0.0f;
        continue;
      }
  ...
  void rebuild_dynamic_overlay()
  {
    ...
    for (std::size_t i = 0; i < dynamic_overlay_.size(); ++i) {
      if (static_map_[i] == 100) {
        continue;
      }
      if (dynamic_score_[i] >= dynamic_lock_threshold_) {
  ...
  float usable_max_range_{3.5f};
  float hit_range_margin_{0.05f};
  float scan_angle_offset_{0.0};
