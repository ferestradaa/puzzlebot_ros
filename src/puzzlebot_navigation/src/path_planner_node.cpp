  ...
  private:
      ...
      const geometry_msgs::msg::PoseStamped & goal_msg)
    {
      ...
      const GridCell start_cell_raw = world_to_grid(rx, ry);
      const GridCell goal_cell_raw  = world_to_grid(gx, gy);

      const GridCell start_cell = nearest_free_cell(start_cell_raw);
      const GridCell goal_cell  = nearest_free_cell(goal_cell_raw);
    ...
    GridCell nearest_free_cell(const GridCell & cell, int max_radius = 8) const
    {
        ...
        for (int r = 1; r <= max_radius; ++r) {
            for (int dx = -r; dx <= r; ++dx) {
                for (int dy = -r; dy <= r; ++dy) {
                    ...
                    const GridCell candidate{cell.first + dx, cell.second + dy};
                    if (is_cell_free(candidate)) return candidate;
                }
            }
        }
        return cell;
