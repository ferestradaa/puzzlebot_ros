...
#Bajado de 384 a 320 — nano no necesita más resolución
...
def build_session(model_path: str, cache_dir: str) -> ort.InferenceSession:
    ...
    so.log_severity_level = 3  # olo errores, reduce overhead de logging
    so.graph_optimization_level = ort.GraphOptimizationLevel.ORT_ENABLE_ALL
    # limitar threads CPU — dejar cores para ROS2 y navegación
...
class PalletNode(Node):
    def __init__(self):
        super().__init__('pallet_inference_gpu')
        self.bridge = CvBridge()

        self.get_logger().info(f'Modelo: {MODEL_PATH}')
        self.get_logger().info('Cargando ONNX con TensorRT EP...')
        ...
        # Buffer reutilizable — evita malloc cada frame
    ...
    def preprocess(self, bgr):
        """Preprocessing in-place sobre buffer reutilizable."""
        img = cv2.resize(bgr, (INPUT_W, INPUT_H))
        # cvtColor + normalize en una sola op con división vectorizada
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
        np.divide(img, 255.0, out=self._input_buf[0].transpose(1, 2, 0),
                  casting='unsafe')
        # transpose ya está implícito por la vista — asegurar contiguous
    ...
    def cb(self, msg: Image):
        ...
        # Matmul vectorizado — todas las máscaras de una vez
        protos = out1[0].reshape(NUM_MASK_COEFFS, -1)       # (32, proto_h*proto_w)
        raw = mask_coeffs @ protos                           # (N, proto_h*proto_w)
        masks_proto = 1.0 / (1.0 + np.exp(-raw))
        #masks_proto = masks_proto.reshape(-1, PROTO_H, PROTO_W)

        _, _, proto_h, proto_w = out1.shape
        masks_proto = masks_proto.reshape(-1, proto_h, proto_w)
        combined_proto = np.max(masks_proto, axis=0)        # (PROTO_H, PROTO_W)
        combined_proto = (combined_proto > MASK_THRESH).astype(np.uint8) * 255
        combined = cv2.resize(combined_proto, (orig_w, orig_h),
                              interpolation=cv2.INTER_NEAREST)  # ✅ más rápido
